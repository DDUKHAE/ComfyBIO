...
class CS5MetagenomicsPlugin(DomainPlugin):
    ...
    def domain_description(self) -> str:
        return "Shotgun metagenomics and 16S amplicon analysis — CS5"

    def get_tsr(self) -> DomainTSR:
        return load_domain_tsr("metagenomics")
    ...
    def load_gold(self, query_id: str) -> TieredGold:
        ...
        if not path.exists():
            raise FileNotFoundError(
                f"No gold criteria for query '{query_id}': {path}"
            )
    ...
    def run_workflow(self, query: HeldOutQuery) -> dict:
        raise NotImplementedError(
            "run_workflow requires domain-specific execution environment"
        )
